...
export default function EditLessonPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    params.then((p) => {
      setLessonId(p.id);
      // നിലവിലുള്ള ലെസൻ ഡാറ്റ ഫെച്ച് ചെയ്യുന്നു
  ...
  // കമ്പ്യൂട്ടർ/മൊബൈൽ മെമ്മറിയിൽ നിന്നുള്ള ഇമേജ് അപ്‌ലോഡ് ഹാൻഡിൽ ചെയ്യാൻ
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.success) {
        setImageUrl(data.url); // അപ്‌ലോഡ് ചെയ്ത ചിത്രത്തിന്റെ ലിങ്ക് സെറ്റ് ചെയ്യുന്നു
      } else {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      router.push('/admin/lessons'); // അഡ്മിൻ ലിസ്റ്റിലേക്ക് റീഡയറക്ട് ചെയ്യുന്നു
    } else {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white shadow-md rounded-lg mt-6">
      ...
      <form onSubmit={handleUpdate} className="space-y-4">
        ...
        {/* കമ്പ്യൂട്ടർ/മൊബൈലിൽ നിന്ന് ഇമേജ് അപ്‌ലോഡ് ചെയ്യാനുള്ള സൗകര്യം */}
        <div>
          ...
          {/* ഇമേജ് ലിങ്ക് മാനുവലായി നൽകാനും എഡിറ്റ് ചെയ്യാനുമുള്ള ഇൻപുട്ട് */}
          <input 
            type="text" 
